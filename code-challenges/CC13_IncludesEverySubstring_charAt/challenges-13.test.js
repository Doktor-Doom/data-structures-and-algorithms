...
const findHappiness = (arr) => {
  // Solution code here...
  let answer = arr.filter(happy => {
    let happyString = happy.match(/\:\)/g);return happyString;}); return answer;
};
...
const standardizePhoneNumbers = (arr) => {
  ...
  // let pineapple = arr.map(number => {
  //   let areaCode = number.substring(1, 4);
  //   let firstThree = number.substring(6, 9);
  //   let lastFour = number.substring(10, 14);
...
const findAnything = (arr, target) => {
  // Solution code here...
  return arr.filter(pineapple => pineapple.includes(target));
};
...
const findEvery = (arr, target) => {
  // Solution code here...
  return arr.every(pineapple => pineapple.includes(target));
};
