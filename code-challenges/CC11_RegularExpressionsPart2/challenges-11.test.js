...
const validateEmail = (email) => {
  ...
  // above is a standard email format validation without the final point of the test.
  let regEx = /\b^[^n][a-zA-Z.]+[^.]@[a-zA-Z]+\.[a-z]{3}$\b/g;
  let validateThis = regEx.test(email);
  return validateThis;
};
...
//   let regex = /ˆ([(][\d]{3}[)]|[\d]{3})[ -]?\d{4}$/g;
//   let newPhone = regex.test(phoneNumber);
//   return newPhone;
};
