...
const createServer = () => {
  ...
  function greeting(req, res) {
    res.send('HelloPineapple');
  }
  app.get('/aboutme', bio);
  function bio(req, res) {
    res.send('I like Pineapple');
  }
  app.get('/favoritefoods', favoriteNoms);
  function favoriteNoms(req, res) {
    res.send('Pineapples');
  }
  var server = app.listen(3301, function () {
    var port = server.address().port;
    console.log('Pineapples listening at port', port);
  });
  return server;
};
...
const count = (target, input) => {
  // Solution code here...
  let counter = 0;
  let answer = input.map( (currentValue) => {
    let innerAnswer = currentValue.map( (innerCurrentValue) => {
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  return (input.map(arr => arr.filter (dividedBy => { dividedBy;
    if(typeof dividedBy === 'number'){
      return dividedBy % 5 === 0;
    } else {
      return false;
    }
