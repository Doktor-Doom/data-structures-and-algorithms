...
const templateWithJQuery = () => {
  ...
  $(starWarsPeople).each( index => {
    const $clonesAttack = $('#template').clone();

    $clonesAttack.find('h2').text(starWarsPeople[index].name);
    $clonesAttack.find('h3').text(starWarsPeople[index].height);
    $clonesAttack.find('p').text(starWarsPeople[2].eye_color);

    $('main').append($clonesAttack);
  });
};
...
const listFoods = (recipe) => {
  ...
  arr.forEach(ingredients => {
    let arrTwo = ingredients.split(' ');
    arrTwo.splice(0, 2);
    result.push(arrTwo.join(' '));
  });
  return result;
};
