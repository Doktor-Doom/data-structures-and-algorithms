...
const mapCurrentEvents = () => {
  // Solution code here...
  return currentEvents.news.map(pineapple => new Events(pineapple));
};
...
const countNumberOfElements = (arr) => {
  // Solution code here...
  const pineapple = arr.reduce((accumulator) => {
    accumulator++;
    return accumulator;
  }, 0);
  return pineapple;
...
const returnNames = (arr) => {
  // Solution code here...
  const pineapple = arr.reduce((acc, val) => {
    acc.push(val.name);
    return acc;
  }, []);
  return pineapple;
...
const reversedString = (str) => {
  // Solution code here...
  const pineapple = str.split('');
  let grilledPineapple = pineapple.reduce( (val, acc) => {
    return acc + val;
  }, '');
  return grilledPineapple;
};
